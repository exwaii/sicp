...
; number of steps is log3(n), or O(logn), since you divide by 3 until you reach 0.01
; space should be constant (?) as angle is no longer used after being passed to the next sine call
; i looked it up, it's called tail recursion optimisation 
; otherwise log(n) if it uses new space for each angle, angle/3 etc